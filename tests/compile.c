...
compiler_set_qualname(compiler *c)
{
    ...
    if (stack_size > 1)
    {
        ...
        if (u->u_scope_type == COMPILE_SCOPE_FUNCTION || u->u_scope_type == COMPILE_SCOPE_ASYNC_FUNCTION || u->u_scope_type == COMPILE_SCOPE_CLASS)
        {
        ...
        if (!force_global)
        {
            if (parent->u_scope_type == COMPILE_SCOPE_FUNCTION || parent->u_scope_type == COMPILE_SCOPE_ASYNC_FUNCTION || parent->u_scope_type == COMPILE_SCOPE_LAMBDA)
            {
...
int _PyCompile_EnterScope(compiler *c, identifier name, int scope_type,
...
void _PyCompile_ExitScope(compiler *c)
{
...
int _PyCompile_PushFBlock(compiler *c, location loc,
...
void _PyCompile_PopFBlock(compiler *c, fblocktype t, jump_target_label block_label)
{
    struct compiler_unit *u = c->u;
...
void _PyCompile_DeferredAnnotations(compiler *c,
...
int _PyCompile_GetRefType(compiler *c, PyObject *name)
{
...
int _PyCompile_LookupCellvar(compiler *c, PyObject *name)
{
    assert(c->u->u_metadata.u_cellvars);
    return dict_lookup_arg(c->u->u_metadata.u_cellvars, name);
}

int _PyCompile_LookupArg(compiler *c, PyCodeObject *co, PyObject *name)
{
...
int _PyCompile_TweakInlinedComprehensionScopes(compiler *c, location loc,
                                               ...
                                               _PyCompile_InlinedComprehensionState *state)
{
    ...
    while (PyDict_Next(entry->ste_symbols, &pos, &k, &v))
    {
        ...
        // either way.)
        if ((scope != outsc && scope != FREE && !(scope == CELL && outsc == FREE)) || in_class_block)
        {
...
int _PyCompile_RevertInlinedComprehensionScopes(compiler *c, location loc,
                                                _PyCompile_InlinedComprehensionState *state)
{
    c->u->u_in_inlined_comp--;
...
int _PyCompile_AddDeferredAnnotation(compiler *c, stmt_ty s,
 ...
 */
int _PyCompile_Error(compiler *c, location loc, const char *format, ...)
{
    va_list vargs;
...
/* Emits a SyntaxWarning and returns 0 on success.
   ...
   and returns -1.
*/
int _PyCompile_Warn(compiler *c, location loc, const char *format, ...)
{
...
int _PyCompile_FutureFeatures(compiler *c)
{
...
int _PyCompile_OptimizationLevel(compiler *c)
{
    return c->c_optimize;
}

int _PyCompile_IsInteractiveTopLevel(compiler *c)
{
...
int _PyCompile_ScopeType(compiler *c)
{
    return c->u->u_scope_type;
}

int _PyCompile_IsInInlinedComp(compiler *c)
{
...
int _PyCompile_AstPreprocess(mod_ty mod, PyObject *filename, PyCompilerFlags *cf,
...
#define SET_METADATA_INT(key, value)                      \
    do                                                    \
    {                                                     \
